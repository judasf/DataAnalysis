...
public class RoleInfo : IHttpHandler, IRequiresSessionState
{
    ...
    /// <summary>
    /// 获取RoleInfo(经费级别表)数据page:1 rows:10 sort:id order:asc
    ...
    {
        DataSet ds = SqlHelper.ExecuteDataset(SqlHelper.GetConnection(), CommandType.Text, "select RoleID,RoleName from roleinfo  where roleid<>0  order by roleid");
        Response.Write(JsonConvert.CreateComboboxJson(ds.Tables[0]));
    }
    /// <summary>
    ///  添加新用户时 生成roleinfo表的combobox使用的json字符串
    /// </summary>
    public void GetRoleInfoComboboxForAdd()
    {
        DataSet ds = SqlHelper.ExecuteDataset(SqlHelper.GetConnection(), CommandType.Text, "select RoleID,RoleName from roleinfo  where roleid<>0  and roleid<>31 and roleid<>32 order by roleid");
        Response.Write(JsonConvert.CreateComboboxJson(ds.Tables[0]));
    }
