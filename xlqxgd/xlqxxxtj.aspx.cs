...
public partial class xlqxxxtj : System.Web.UI.Page
{
    public string year;
    ...
    {
        DataSet ds = DirectDataAccessor.QueryForDataSet("select distinct DateName(year,qxrq)as yearstr from xlqxxx ");
    ...
    private string GetSqlStr()
    {
        ...
        if (Session["roleid"] != null && Session["deptname"] != null && (Session["roleid"].ToString() == "1" || Session["roleid"].ToString() == "2"))
        {
            ...
            dwStr = " where  a.qxdw='" + Session["deptname"].ToString() + "'";
        }
        else
        {
            if (Request.QueryString["dw"] != null)
            {
                ...
                dwStr = " where  a.qxdw='" + Request.QueryString["dw"].ToString() + "'";
            }
        }
      
        //循环生成sql语句
        StringBuilder sql = new StringBuilder("select a.qxdw"); //完整的sql
        ...
        for (int i = 1; i < 13; i++)
        {
            s = i.ToString();
            month = i < 10 ? "-0" + s : "-" + s;
            whereStr = "where substring(qxrq,0,8)='" + year + month + "'";
            ...
            joinStr.Append("select qxdw,count(id) as num,sum(ssje) as amount from xlqxxx ");
            joinStr.Append(whereStr);
            joinStr.Append(" group by qxdw,substring(qxrq,0,8) )");
            joinStr.Append(" as m" + s + " on a.qxdw=m" + s + ".qxdw ");
        }
        sql.Append(zdStr);
        sql.Append(" from (select qxdw from xlqxxx group by qxdw) as a ");
        sql.Append(joinStr);
        sql.Append(dwStr);
        return sql.ToString();
    }
    ...
    private void xlsGridview(DataTable dt, string xlsName)
    {
        ...
        xf.Font.Bold = true;
        //设置月份
        string[] colums1 = { "抢修单位", "1月", "", "2月", "", "3月", "", "4月", "", "5月", "", "6月", "", "7月", "", "8月", "", "9月", "", "10月", "", "11月", "", "12月", "" };
