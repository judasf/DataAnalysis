...
public class SqlChecker
{
    ...
    private const string StrKeyWord = @"select|insert|delete|from|drop table|update|truncate|exec master|netlocalgroup administrators|net user|or|and|asc(|mid(|char(|xp_cmdshell|exec master";
    ...
    public SqlChecker(HttpRequest _request, HttpResponse _response,HttpUtility _server)
    {
    ...
    public string Msg
    {
        get
        {
            ...
            if (this.safeUrl == String.Empty)
                msg += " window.top.location.href = '" + request.RawUrl + "'";
            else
                msg += " window.top.location.href = '" + safeUrl + "'";
            msg += "</script>";
            return msg;
        }
    }
    ...
    public bool CheckRequestQuery()
    {
        ...
        if (request.QueryString.Count != 0)
        {
            ...
            foreach (string queryName in this.request.QueryString)
            {
                ...
                if (CheckKeyWord(HttpUtility.HtmlDecode(request.QueryString[queryName])))
                {
                    //只要存在一个可能出现Sql注入的参数,则直接退出
                    result = true;
                    break;
    ...
    public bool CheckRequestForm()
    {
        ...
        if (request.Form.Count > 0)
        {
            ...
            foreach (string queryName in this.request.Form)
            {
                ...
                if (CheckKeyWord(HttpUtility.HtmlDecode(request.Form[queryName])))
                {
                    //只要存在一个可能出现Sql注入的参数,则直接退出
                    result = true;
                    break;
