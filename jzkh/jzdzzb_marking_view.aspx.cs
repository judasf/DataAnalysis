...
public partial class jzkh_jzdzzb_marking_view : System.Web.UI.Page
{
    ...
    {
        double total=0;
        StringBuilder sql = new StringBuilder("select '基站断站指标 （100分）'as pclass,row_number() over(order by b.id) as rowid, b.id,a.classname,itemname,markname,markstd");
        sql.Append(",c.score,c.memo,c.markingdept,c.markingtime ");
        sql.Append("from ((jzkh_class as a join  jzkh_item as b ");
        sql.Append("on b.classid=a.id and a.parentid=1) join jzkh_marking as c ");
