...
public partial class jzkh_jzdzzb_marking : System.Web.UI.Page
{
    ...
    {
        string sql = "select '基站断站指标 （100分）'as pclass,row_number() over(order by b.id) as rowid, b.id,a.classname,itemname,markname,markstd";
        sql += " from jzkh_class as a join  jzkh_item as b ";
        sql += "on b.classid=a.id and a.parentid=1";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        sql.Append(" Update  jzkh_score set dzzb_score=" + (100-total) * ratio + " where deptname='" + deptname .Text+ "' ");
        sql.Append(" and scoredate='" + scoredate .InnerText+ "'");
        sql.Append(" ELSE ");
        sql.Append(" Insert into  jzkh_score(deptname,scoredate,dzzb_score) values('"+deptname.Text+"','"+scoredate.InnerText+"',"+(100-total)*ratio+")");
