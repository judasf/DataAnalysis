...
public partial class jzkh_jzewjc_marking : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                //判断权限;9:基站考核用户：zhaoziqiang,考核该项
                if (Session["roleid"] == null || Session["roleid"].ToString() != "9" || Session["uname"].ToString() != "zhaoziqiang")
    ...
    private void NewsBind()
    {
        ...
        sql += "on b.classid=a.id and a.id=5";
        DataSet ds = DirectDataAccessor.QueryForDataSet(sql);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        sql.Append(" Update  jzkh_score set ewjc_score=" + total + " where deptname='" + deptname.Text + "' ");
        sql.Append(" and scoredate='" + scoredate .InnerText+ "'");
        sql.Append(" ELSE ");
        sql.Append(" Insert into  jzkh_score(deptname,scoredate,ewjc_score) values('" + deptname.Text + "','" + scoredate.InnerText + "'," + total + ")");
