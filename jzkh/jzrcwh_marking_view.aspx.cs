...
public partial class jzkh_jzrcwh_marking_view : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    deptname.InnerHtml = Server.UrlDecode(Request.QueryString["dept"]);
                    NewsBind();
    ...
    {
        double total = 0;
        StringBuilder sql = new StringBuilder("select '日常维护管理及考核（100分）' as pclass, b.id,row_number() over(order by b.id) as rowid,a.classname,itemname,markname,markstd,b.memo as addmemo");
        sql.Append(",c.score,c.memo,c.markingtime ");
        sql.Append("from ((jzkh_class as a join  jzkh_item as b ");
        sql.Append("on b.classid=a.id and a.parentid=3) join jzkh_marking as c ");
        sql.Append("on b.id=c.itemid and c.deptname='" + deptname.InnerHtml + "' and c.scoredate='" + scoredate.InnerText + "' ");
        //if (deptname.InnerHtml != "市区")
        //{
