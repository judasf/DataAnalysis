...
public partial class jzkh_jzwykh_marking : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                //判断权限;9:基站考核;用户：renerhong,考核该项
                if (Session["roleid"] == null || Session["roleid"].ToString() != "9" || Session["uname"].ToString() != "renerhong")
    ...
    {
        string sql = "select row_number() over(order by b.id) as rowid, b.id,a.classname,itemname,markstd";
        sql += " from jzkh_class as a join  jzkh_item as b ";
        sql += "on b.classid=a.id and a.id=4";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        sql.Append(" Update  jzkh_score set wykh_score=" + (100 - total) * ratio + " where deptname='" + deptname.Text + "' ");
        sql.Append(" and scoredate='" + scoredate .InnerText+ "'");
        sql.Append(" ELSE ");
        sql.Append(" Insert into  jzkh_score(deptname,scoredate,wykh_score) values('" + deptname.Text + "','" + scoredate.InnerText + "'," + (100 - total) * ratio + ")");
