...
<% 
    /** 
     * 运维物料物料领取
     *MaintainMaterial_StockDraw表操作对话框
     *
     */
...
<script type="text/javascript">
    ...
    var onFormSubmit = function ($dialog, $grid) {
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '您确定要提交领料信息？提交后则不能修改！', function (r) {
                if (r) {
                    var url = 'ajax/Srv_MaintainMaterial.ashx/SaveUnitStockDrawInfo';
                    //判断领料单上传
                    if ($('#report').val() == "") {
    ...
    var setOrderStock = function (index, id) {
        $('#storeorderno' + index).combogrid({
            url: 'ajax/Srv_MaintainMaterial.ashx/GetOrderCombogridForStockOut',
            panelWidth: 280,
            panelHeight: 140,
    ...
    var addList = function () {
        var index = $('#index').val();
        index++;
        var insertEle = $('<tr align="center"><td> <input type="text" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="url:\'../ajax/Srv_MaintainMaterial.ashx/GetClassInfoComboboxForStockOut\',valueField: \'id\',textField: \'text\',panelHeight:\'auto\',editable: false,required:true,onSelect:function(rec){ var url = \'../ajax/Srv_MaintainMaterial.ashx/GetTypeInfoComboboxForStockOut?classname=\'+encodeURIComponent(rec.text);$(\'#typeid' + index + '\').combobox(\'setValue\',\'\').combobox(\'reload\', url); }"/> </td><td><input type="text" id="typeid' + index + '" name="typeid" style="width: 280px;" class="combo easyui-combobox" data-options="valueField: \'id\',textField: \'text\',editable: false,panelHeight: \'200\',required:true,onSelect:function(rec){setOrderStock(' + index + ',rec.id);}" /></td> <td><input type="text" id="storeorderno' + index + '" name="storeorderno" style="width: 160px;" /></td><td><input type="text" id="amount' + index + '" name="amount" class="inputBorder easyui-numberbox" style="width: 50px" data-options="min:0,required:true" onblur="checkStock(' + index + ',this);"/></td><td><label id="stock' + index + '"></label></td><td><label id="units' + index + '"></label></td><td><img src="../../Script/easyui/themes/icons/edit_remove.png" onclick="delList(this);" style="cursor: pointer;" /></td></tr>').insertBefore($('#memoTr'));
        $('#index').val(index);
    ...
    $(function () {
        ...
        $("#file_upload").uploadify({
            ...
            //在文件上传时需要一同提交的数据
            'formData': { 'floderName': 'MaintainMaterial' },
            'buttonText': '浏览',
            //flash
...
<form method="post">
    ...
    <table class="doc-table" id="stockList">
        ...
        <tr>
            <td style="text-align: left" colspan="7">领料日期：
                ...
                <input id="lldw" type="text" name="lldw" style="width: 140px;" class="combo easyui-combobox" data-options="
                    ...
                    required:true,
                    panelHeight: 'auto',
                    url: 'ajax/Srv_MaintainMaterial.ashx/GetMaintainMaterial_lldwCombobox',
                    onSelect:function(rec){ var url = '../ajax/Srv_MaintainMaterial.ashx/GetllrComboboxForStockDraw?lldw='+encodeURIComponent(rec.text);$('#llr').combobox('setValue','').combobox('reload', url); }
                      " />
        ...
        <tr align="center">
            <td>
                <%--<select id="classname" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="panelHeight:'auto',editable: false,required:true,onSelect:function(rec){ var url = '../ajax/Srv_MaintainMaterial.ashx/GetTypeInfoComboboxForStockOut?classname='+encodeURIComponent(rec.value);$('#typeid1').combobox('setValue','').combobox('reload', url); }">
                    <option value=""></option>
                    <option>光缆</option>
                    <option>电力电缆</option>
                    <option>双绞线</option>
                ...
                <input type="text" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="
                    url:'../ajax/Srv_MaintainMaterial.ashx/GetClassInfoComboboxForStockOut',
                    valueField: 'id',
                    textField: 'text',
                    panelHeight:'auto',
                    editable: false,
                    ...
                    onSelect:function(rec){
                     var url = '../ajax/Srv_MaintainMaterial.ashx/GetTypeInfoComboboxForStockOut?classname='+encodeURIComponent(rec.text);
