...
<script type="text/javascript">
        ...
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '您确定要提交调拨信息？提交后则不能修改！', function (r) {
                if (r) {
                    ...
                    var url = 'ajax/Srv_MaintainMaterial.ashx/SaveUnitAllotStockInfo';
    ...
    $(function () {
        if ($('#id').val().length > 0) {
            ...
            $.post('../ajax/Srv_MaintainMaterial.ashx/GetMaintainMaterialStockById', {
                ID: $('#id').val()
            }, function (result) {
                if (result.rows[0].id != undefined) {
                    ...
                    $('#allotunitid').combobox({
                        ...
                        panelHeight: 'auto',
                        required: true,
                        url: 'ajax/Srv_MaintainMaterial.ashx/GetAllotUnitInfoCombobox?id=' + encodeURIComponent(result.rows[0].unitname)
                    });
