    ...
    <script type="text/javascript">
        ...
        var addFun = function () {
            var dialog = parent.parent.$.modalDialog({
                ...
                height: 500,
                iconCls: 'icon-add',
                href: 'MaintainMaterial/dialogop/UnitStockMana_OP.aspx',
                buttons: [{
                    text: '提交',
                    handler: function () {
        ...
        var removeFun = function (id) {
                var dialog = parent.$.modalDialog({
                    ...
                    height: 500,
                    iconCls: 'icon-remove',
                    href: 'MaintainMaterial/dialogop/UnitStockOut_OP.aspx',
                    buttons: [{
                        text: '提交',
                        handler: function () {
                            parent.onFormSubmit(dialog, grid);
        ...
        var drawFun = function (id) {
            var dialog = parent.$.modalDialog({
                ...
                height: 500,
                iconCls: 'icon-remove',
                href: 'MaintainMaterial/dialogop/UnitStockDraw_OP.aspx',
                buttons: [{
                    text: '提交',
                    handler: function () {
                        parent.onFormSubmit(dialog, grid);
        ...
        var allotFun = function () {
            ...
            else if (row.amount == 0)
            {
                ...
                var dialog = parent.parent.$.modalDialog({
                    ...
                    height: 450,
                    iconCls: 'icon-add',
                    href: 'MaintainMaterial/dialogop/UnitStockAllot_OP.aspx?id='+row.id,
                    buttons: [{
                        text: '提交',
                        handler: function () {
                            parent.onFormSubmit(dialog, grid);
        ...
        var exportStock = function () {
            jsPostForm('../ajax/Srv_MaintainMaterial.ashx/ExportUnitStock', $.serializeObject($('#searchForm')));
        };
        $(function () {
            //初始化型号下拉框
            $('#typeid').combobox({
            ...
            grid = $('#grid').datagrid({
                title: '运维物料库存表',
                url: '../ajax/Srv_MaintainMaterial.ashx/GetMaintainMaterialUnitStock',
                striped: true,
                rownumbers: true,
                pagination: true,
                singleSelect: true,
...
<body class="easyui-layout" style="overflow: hidden;">
        ...
        <form id="searchForm" style="margin: 0;">
            <table>
                <tr>
                    ...
                    <td>
                        <select id="unitname" class="combo easyui-combobox" name="unitname" style="width: 220px;" data-options="panelHeight:'auto',editable: false,onSelect:function(rec){ var url = '../ajax/Srv_MaintainMaterial.ashx/GetMaintainMaterial_UnitAreaComboboxAll?unitname='+encodeURIComponent(rec.value);$('#areaid').combobox('reload', url); }">
                            <%if (roleid == 2 )
                                { %>
    ...
    <div data-options="region:'center',fit:true,border:false">
        <p class="sitepath">
            <b>当前位置：</b>运维物料管理 > <a href="javascript:void(0);">运维物料管理——物料库存管理</a>
        </p>
