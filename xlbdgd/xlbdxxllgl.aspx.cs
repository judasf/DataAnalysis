...
public partial class xlbdxxllgl : System.Web.UI.Page
{
    ...
    {
        string sql;
        sql = "select b.* from xlbdxx as a join xlbdxx_llmx as b on a.id=b.bdid ";
        if (Session["roleid"] != null && Session["roleid"].ToString() == "2")
            sql += " and  bddw='" + Session["deptname"] + "' ";
        ...
        if (Request.QueryString["bdid"] != null)
        {
            ...
            whereStr += " bdid like '%" + Request.QueryString["bdid"].ToString() + "%' and";
        }
        if (whereStr != "where")
        {
