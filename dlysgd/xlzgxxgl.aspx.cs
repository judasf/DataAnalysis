...
{    
            ...
            else
            {
                ...
                if (Session["roleid"] != null && Session["roleid"].ToString() == "1")
                {
                    ...
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条电缆延伸未确认电缆延伸完结，请及时处理！')", true);
                }
                //库管
                if (Session["roleid"] != null && Session["roleid"].ToString() == "2")
                {
                    ...
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条电缆延伸区域领料未出库，请及时处理！')", true);
                }
                ...
                {
                    //提示未指派区域维护
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where isnull(qywh,'')=''  and len(id)=13 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条电缆延伸通知未派单，请及时处理！')", true);
                    //提示未确认区域维护领料
                    DataSet ds1 = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where  zgll=1 and wbqr=0  and len(id)=13 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds1.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds1.Tables[0].Rows[0][0].ToString() + "条电缆延伸未确认区域维护领料单，请及时处理！')", true);
                    //提示未设置完结时间
                    DataSet ds2 = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where kgck=1 and isnull(wjsj,'')=''  and len(id)=13 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds2.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds2.Tables[0].Rows[0][0].ToString() + "条电缆延伸未设置完结时间，请及时处理！')", true);
                }
                //区域维护
                if (Session["roleid"] != null && Session["roleid"].ToString() == "7")
                {
                    //提示未录入电缆延伸措施
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where isnull(zgcs,'')=''  and len(id)=13 and qywh='" + Session["uname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条电缆延伸未录入电缆延伸措施，请及时处理！')", true);
                //区域维护提示未录入电缆延伸领料
                    DataSet ds1 = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where zgll=0 and isnull(zgcs,'')<>''  and len(id)=13 and qywh='" + Session["uname"].ToString() + "'");
                    if (ds1.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds1.Tables[0].Rows[0][0].ToString() + "条电缆延伸未领料，请及时处理！')", true);
                    //区域维护提示未设置电缆延伸完结
                ...
                {
                    //提示未确认区域维护领料单
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from dlysxx where  wbqr=1 and xgqr=0    and len(id)=13 and  zgtd=0 and pfdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条电缆延伸未确认区域维护领料单，请及时处理！')", true);
        ...
        if (Request.QueryString["gdzt"] != null)
        {
            ...
            switch (gdzt_str){
                ...
                case "zgwj":
                    ...
                    /*复查流程取消
                case "sffc":
                    whereStr += " isnull(fcsj,'')='' and isnull(qrwjsj,'')<>'' and";
                    break;
                     * */
                case "sftl":
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        outputFileName += "线路电缆延伸信息明细表.xls";
        ...
        dt.Columns[8].ColumnName = "电缆延伸区域";
        dt.Columns[9].ColumnName = "存在问题";
        dt.Columns[10].ColumnName = "电缆延伸要求";
        dt.Columns[11].ColumnName = "电缆延伸时限";
        dt.Columns[12].ColumnName = "区域维护";
        dt.Columns[13].ColumnName = "电缆延伸措施";
        dt.Columns[14].ColumnName = "电缆延伸人";
        dt.Columns[15].ColumnName = "退单原因";
        dt.Columns[16].ColumnName = "完结时间";
        dt.Columns[17].ColumnName = "确认完结时间";
        dt.Columns[18].ColumnName = "电缆延伸备注";
    ...
    /// 处理区域维护电缆延伸措施
    /// </summary>
    /// <param name="id">电缆延伸id</param>
    /// <param name="zgcs">电缆延伸措施</param>
    /// <param name="qywh">区域维护</param>
    /// <returns></returns>
    public string handleZGCS(string id, string zgcs, string qywh)
    {
        string str = "";
        if (zgcs != "")//已录入电缆延伸措施
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情信息'>已录入</a>";
        else //未录入电缆延伸措施
        {
            //角色判断，7：区域维护有权录入电缆延伸措施
            if (Session["roleid"] != null && Session["roleid"].ToString() == "7")
            {
                if (qywh == "")//未指定区域维护
                    str = "<a href=\"javascript:alert('该电缆延伸未指定区域维护，不能录入电缆延伸措施！');\" class='b_blue'>未录入</a> ";
                else //已指定区域维护
                    str = "<a class='b_blue' href=\"xlzgxxzgcs.aspx?id=" + id + "\" title='点击录入电缆延伸措施'>未录入</a>";
            }
            else //非区域维护
    ...
    public string handleQYLL(string id, string zgll, string zgcs)
   {
       string str="";
           ...
           else //未领料
           {
               ...
               {
                   if (zgcs == "")//未录入电缆延伸措施
                       str = "<a href=\"javascript:alert('该电缆延伸未录入电缆延伸措施，不能领料！');\" class='b_blue'>未领料</a> ";
                   else //已录入电缆延伸措施
    ...
    public string handleWBQR(string id, string wbqr, string zgll)
   {
       ...
       if (wbqr == "1")//外包已确认
       {
           str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已确认</a>";
       }
       else
       {//外包未确认
           //角色判断，3：外包单位有权确认电缆延伸措施和区域维护领料信息
           if (Session["roleid"] != null && Session["roleid"].ToString() == "3")//是外包单位
           {
               if (zgll == "0")//未领料
                   str = "<a href=\"javascript:alert('该电缆延伸还未领料，不能进行领料工单确认！');\" class='b_green'>未确认</a> ";
               else
    ...
    /// <summary>
    /// 处理线管对电缆延伸措施和料单确认（针对市区）
    /// </summary>
    /// <param name="id">电缆延伸编号</param>
    /// <param name="xgqr">线管确认</param>
 /// <param name="zgtd">电缆延伸退单</param>
    /// <param name="wbqr">外包确认</param>
    /// <returns></returns>
    public string handleXGQR(string id, string xgqr,string zgtd, string wbqr)
    {
        ...
        {
            str = "<a class='b_red'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已退单</a>";
        }
        else //线管未退单
        {
            if (xgqr == "1")//线管已确认
    ...
    public string handleKGQR(string id, string kgck, string xgqr)
    {
        ...
        if (kgck == "1")//库管已确认出库
        {
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已出库</a>";
        }
        else
        {//库管未确认出库
    ...
    public string handleWBZGWJ(string id, string wjsj, string kgck)
    {
        ...
        if (wjsj != "")//外包已录入完结时间
        {
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已完成</a>";
        }
        else
        {//外包未确认
            //角色判断，3||7：外包单位和区域维护有权录入电缆延伸完结时间
            if (Session["roleid"] != null &&( Session["roleid"].ToString() == "3"|| Session["roleid"].ToString() == "7"))//是外包或区域主管单位
            {
                if (kgck == "0")//领料未出库
                    str = "<a href=\"javascript:alert('该电缆延伸领料还未出库，不能进行设置电缆延伸完结！');\" class='b_green'>未完成</a> ";
                else
                    str = "<a class='b_green' href=\"xlzgxxwbzgwj.aspx?zgid=" + id + "\" title='点击进行外包电缆延伸完结'>未完成</a>";
            }
            else//非外包
                str = "<span class='b_green'>未完成</a>";
        }
        return str;
    }
    /// <summary>
    ...
    public string handleXGQRWJ(string id, string qrwjsj, string zgbz,string wjsj)
    {
        string str = "";
        if (qrwjsj != "")//已确认完结
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已完结</a>";
        else
        {
            //派单人有权限确认电缆延伸完结
            ...
            {
                if (wjsj == "")//外包未设置完结时间
                    str = "<a href=\"javascript:alert('该电缆延伸未完成，不能完结！');\" class='b_red'>未完结</a> ";
                else //外包已设置完结时间
                {
                    if (zgbz == "")
                        str = "<a class='b_red' href=\"xlzgxxqrzgwj.aspx?zgid=" + id + "\" title='点击录入确认电缆延伸信息'>未完结</a>";
                    else
                        str = "<a class='b_red' href=\"xlzgxxqrzgwj.aspx?bz=1&zgid=" + id + "\" title='点击录入确认电缆延伸信息'>未完结</a>";
                }
            }
            else //非派单人
  ...
  /*
    ...
    /// <param name="zgtl">电缆延伸退料</param>
    /// <returns></returns>
   public string handleFC(string id, string fcr, string qrwjsj)
   {
       string str = "";
       if (fcr != "")//已复查
           str = "<a class='gray'   href=\"dlysxxxq.aspx?zgid=" + id + "\"  title='点击查看电缆延伸详情'>已复查</a>";
       else
       {
           //判断
           if (Session["roleid"] != null && (Session["roleid"].ToString() == "1" || Session["roleid"].ToString() == "5" || Session["roleid"].ToString() == "8"))
           {
               if (qrwjsj == "")//派单人未确认电缆延伸完结
                   str = "<a href=\"javascript:alert('该电缆延伸确认完结，不能复查！');\" class='b_orange'>未复查</a> ";
               else //派单人已确认电缆延伸完结
                   str = "<a class='b_orange' href=\"dlysxxfc.aspx?id=" + id + "\" title='点击录入电缆延伸复查信息'>未复查</a>";
           }
           else //非派单人
