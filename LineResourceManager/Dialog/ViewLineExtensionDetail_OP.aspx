...
<script type="text/javascript">
    ...
    //资料有误，退回施工单位重新上传
    var backToUnit = function ($dialog, $grid) {
        var url = './ajax/Srv_LineResource.ashx/BackToUnitByID';
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '确认退回施工单位重新上传资料？', function (r) {
                if (r) {
                    $.post(url, $.serializeObject($('form')), function (result) {
                        if (result.success) {
                            parent.$.messager.alert('提示', result.msg, 'info');
...
<form>
    <table cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" border="1" style="border-collapse: collapse;" id="detailTable">
        ...
        <tr>
            <td class="left_td">资料录入时间：
            </td>
            <td class="tdinput" colspan="3">
                <span id="inputfiletime"></span>
            </td>
        </tr>
    </table>
</form>
