...
public partial class xlqgllxxxq : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                DataSet ds1 = DirectDataAccessor.QueryForDataSet("select top 1* from xlqgxx_llmx where qgid='" + Request.QueryString["id"].ToString() + "'");
    ...
    {
        string sqlStr = "select * from xlqgxx_llmx  ";
    ...
    {
        string sqlStr = "select  row_number() over(order by id ) as rowid,* from xlqgxx_llmx  where qgid='" + Request.QueryString["id"].ToString()+"'" ;
        DataSet ds = DirectDataAccessor.QueryForDataSet(sqlStr);
