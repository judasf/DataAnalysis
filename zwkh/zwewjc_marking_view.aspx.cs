...
public partial class zwkh_zwewjc_marking_view : System.Web.UI.Page
{
    ...
    {
        double total=0;
        StringBuilder sql = new StringBuilder("select  b.id,row_number() over(order by b.id) as rowid,a.classname,itemname,std,marks,markstd");
        sql.Append(",c.score,c.memo,c.markingdept,c.markingtime ");
        sql.Append("from ((zwkh_class as a join  zwkh_item as b ");
        sql.Append("on b.classid=a.id and a.id=3) join zwkh_marking as c ");
