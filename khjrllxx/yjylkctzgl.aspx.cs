...
public partial class yjylkctzgl : System.Web.UI.Page
{
    ...
    private void BindClass()
    {
        ...
        DataSet ds = DirectDataAccessor.QueryForDataSet("select *  from " + Session["pre"] + "yjylkc_Class  where  classname='客户接入'");
    ...
    private void Binddw()
    {
        string sql = "select ckdw from yjylkc_llmx where classname='客户接入' ";
        ...
        if (Session["roleid"] != null && Session["deptname"] != null && (Session["roleid"].ToString() == "1" || Session["roleid"].ToString() == "2" || Session["roleid"].ToString() == "8"))
            sql += " and ckdw='" + Session["deptname"].ToString() + "'";
        sql += " group by ckdw";
    ...
    private void Bindlldw(string ckdw)
    {
        string sql = "select unitname from  UnitAndDeptRelation  where deptname ='" + ckdw + "'";
        DataSet ds = DirectDataAccessor.QueryForDataSet(sql);
        ddllldw.Items.Clear();
        ddllldw.Items.Add(new ListItem("---------全部---------", "0"));
        foreach (DataRow dr in ds.Tables[0].Rows)
        {
            ddllldw.Items.Add(new ListItem(dr[0].ToString()));
        }
    ...
    private DataTable GetDataTable()
    {
        ...
        if (Request.QueryString["lb"] != null)
        {
            ...
            whereStr += "  classname='" + Server.UrlDecode(Request.QueryString["lb"].ToString()) + "' and";
        }
        ...
        else
        {
            if (Request.QueryString["dw"] != null)
            {
                ...
                Bindlldw(ddldw.Text);
                whereStr += "  ckdw='" + Server.UrlDecode(Request.QueryString["dw"].ToString()) + "' and";
            }
        }
        if (Request.QueryString["lldw"] != null)
        {
            ddllldw.Text = Server.UrlDecode(Request.QueryString["lldw"].ToString());
            whereStr += "  lldw = '" + Server.UrlDecode(Request.QueryString["lldw"].ToString()) + "' and";
        }
        whereStr += "  classname='客户接入' and";
        if (whereStr != "where")
        {
    ...
    private void NewsBind()
    {
        ...
        if (Request.QueryString["lb"] != null)
        {
            condition += "&lb=" + Server.UrlEncode(Request.QueryString["lb"].ToString());
        }
        if (Request.QueryString["dw"] != null)
        {
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        if (ddlClass.Text != "0")
            condition += "&lb=" + Server.UrlEncode(ddlClass.Text);
        if (ddldw.Text != "0")
            condition += "&dw=" + Server.UrlEncode(ddldw.Text);
        if (ddllldw.Text != "0")
            condition += "&lldw=" + Server.UrlEncode(ddllldw.Text);
        if (id.Text != "")
