...
<script type="text/javascript">
    var onFormSubmit = function ($dialog, $grid) {
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '您确定要提交资源入库信息？提交后则不能修改！', function (r) {
                if (r) {
                    var url = 'ajax/Srv_ResourceReuse.ashx/SaveUnitStockInfo';
                    //要post的json数据
                    var postDate = {};
                    //有数据的行编号
                    var rowsNum = 0;
...
<form method="post">
    ...
    <table class="doc-table" id="stockList">
        ...
        <tr align="center">
            ...
            <td>
                 <select id="classname" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="panelHeight:'auto',editable: false,required:true,onSelect:function(rec){ var url = '../ajax/Srv_ResourceReuse.ashx/GetResourceReuse_TypeInfoComboboxForIn?classname='+encodeURIComponent(rec.value);$('#typeid1').combobox('setValue','').combobox('reload', url); }">
                            <option value=""></option>
                            <option>光缆</option>
                            <option>电力电缆</option>
                     <option>双绞线</option>
