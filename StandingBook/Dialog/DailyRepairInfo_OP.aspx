...
<script type="text/javascript">
    var onFormSubmit = function ($dialog, $grid) {
        ...
        if ($('#resForm').form('validate')) {
            parent.$.messager.confirm('询问', '您确认要提交维修台账信息？提交后则不能修改！', function (r) {
                if (r) {
                    var url = 'ajax/Srv_StandingBook.ashx/SaveDailyRepairInfo';
                    if ($('#report').val() == "") {
                    ...
                    //增加限制，使用物料时，才获取数据
                    if ($('#isusematerial').combobox('getText') == '是') {
                        //遍历每一行表格
                    ...
                    //插入是否使用物料
                    postDate['isusematerial'] = $('#isusematerial').combobox('getText');
                    //插入总数据行数
    ...
    var setOrderStock = function (index, id) {
        $('#stockdrawid' + index).combogrid({
            url: 'ajax/Srv_MaintainMaterial.ashx/GetOrderCombogridForDailyRepair',
            panelWidth: 230,
            panelHeight: 140,
    ...
    $(function () {
        ...
        $('#isusematerial').combobox({
            ...
            onSelect: function (rec) {
                ...
                //使用物料时设置显示和必填项
                if (rec.value == '是') {
                    $('#repairMaterialInfo').show()
                    flag = true;
                }
                else {
                    $('#repairMaterialInfo').hide();
                    flag = false;
                }
                for (var i = 1; i <= index; i++) {
                    $('#classname' + i).combobox({ required: flag });
                    $('#typeid' + i).combobox({ required: flag });
                    $('#amount' + i).numberbox({ required: flag });
...
<form method="post" id="resForm">
    <table cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" border="1" style="border-collapse: collapse;">
        ...
        <tr>
            ...
            <td class="tdinput" colspan="3">
                <select id="isusematerial" class="easyui-combobox" name="isusematerial" style="width: 100px;">
                    <option>是</option>
                    <option>否</option>
                </select>
            </td>
        </tr>
        <tr id="repairMaterialInfo">
            <td colspan="4">
                ...
                <table class="doc-table" id="wxylList">
                    ...
                    <tr align="center">
                        <td>
                            <input type="text" class="combo easyui-combobox" name="classname" id="classname1" style="width: 100px;" data-options="
                    url:'../ajax/Srv_MaintainMaterial.ashx/GetClassInfoComboboxForDailyRepair',
                    valueField: 'id',
                    textField: 'text',
                    panelHeight:'auto',
                    editable: false,
                    ...
                    onSelect:function(rec){
                     var url = '../ajax/Srv_MaintainMaterial.ashx/GetTypeInfoComboboxForDailyRepair?classname='+encodeURIComponent(rec.text);
