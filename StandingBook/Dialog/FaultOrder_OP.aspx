...
<script type="text/javascript">
    var onFormSubmit = function ($dialog, $grid) {
        if ($('#resForm').form('validate')) {
            var url;
            if ($('#id').val().length == 0) {
                url = '../ajax/Srv_StandingBook.ashx/SaveFaultOrderInfo';
            } else {
                url = '../ajax/Srv_StandingBook.ashx/UpdateFaultOrderInfo';
            }
            if ($('#report_order').val() == "") {
    ...
    var getFaultOrderNo = function () {
        ...
        $.post('../ajax/Srv_StandingBook.ashx/GetFaultOrderNo', function (result) {
            if (result.success) {
