    ...
    <script type="text/javascript">
        ...
        var addFun = function () {
            var dialog = parent.parent.$.modalDialog({
                ...
                height: 150,
                iconCls: 'icon-add',
                href: 'BaseUnit/dialogop/wlxh_OP.aspx', //将对话框内容添加到父页面
                buttons: [{
                    text: '添加',
                    handler: function () {
        ...
        var editFun = function (id) {
            var dialog = parent.$.modalDialog({
                ...
                height: 150,
                iconCls: 'icon-edit',
                href: 'BaseUnit/dialogop/wlxh_OP.aspx?id=' + id,
                buttons: [{
                    text: '保存',
                    handler: function () {
                        parent.onClassFormSubmit(dialog, grid);
        ...
        var removeFun = function (id) {
            parent.$.messager.confirm('询问', '您确定要删除此记录？', function (r) {
                if (r) {
                    $.post('../ajax/Srv_khjr.ashx/RemovewlxhByID', {
                        id: id
                    }, function (result) {
                        if (result.success) {
        ...
        $(function () {
            grid = $('#grid').datagrid({
                title: '物料型号表',
                url: '../ajax/Srv_khjr.ashx/GetKhjrWlxh',
                striped: true,
                rownumbers: true,
                pagination: true,
                singleSelect: true,
