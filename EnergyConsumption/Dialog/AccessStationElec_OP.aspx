...
<script type="text/javascript">
    var onFormSubmit = function ($dialog, $grid) {
        if ($('#resForm').form('validate')) {
            var url;
            if ($('#id').val().length == 0) {
                url = '../ajax/Srv_EnergyConsumption.ashx/SaveAccessStationElecInfo';
            } else {
                url = '../ajax/Srv_EnergyConsumption.ashx/UpdateAccessStationElecInfo';
            }
    ...
    $(function () {
        ...
        if ($('#id').val().length > 0) {
            ...
            $.post('../ajax/Srv_EnergyConsumption.ashx/GetAccessStationElecByID', {
                ID: $('#id').val()
            }, function (result) {
...
<form method="post" id="resForm">
    <table cellspacing="0" cellpadding="0" bordercolor="#CCCCCC" border="1" style="border-collapse: collapse;">
        ...
        <tr>
            <td class="left_td" valign="top">备注：
            </td>
            <td class="tdinput" colspan="3">
