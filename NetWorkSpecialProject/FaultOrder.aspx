    ...
    <script type="text/javascript">
        ...
        var addOrder = function () {
            var dialog = parent.$.modalDialog({
                ...
                height: 600,
                iconCls: 'icon-add',
                href: 'NetWorkSpecialProject/DialogOP/FaultOrder_OP.aspx',
                buttons: [{
                    text: '提交',
                    handler: function () {
        ...
        var editOrder = function (id) {
            var dialog = parent.$.modalDialog({
                ...
                height: 600,
                iconCls: 'icon-edit',
                href: 'NetWorkSpecialProject/DialogOP/FaultOrder_OP.aspx?id=' + id,
                buttons: [{
                    text: '保存',
                    handler: function () {
                        parent.onFormSubmit(dialog, orderGrid);
        ...
        var removeFun = function (orderno) {
            parent.$.messager.confirm('询问', '您确定要删除该故障信息？', function (r) {
                if (r) {
                    $.post('../ajax/Srv_NetWorkSpecialProject.ashx/RemoveFaultOrderByOrderNo', {
                        orderno: orderno
                    }, function (result) {
                        if (result.success) {
                            $.messager.show({
        ...
        var viewOrderDetail = function (id) {
            ...
            var dialog = parent.$.modalDialog({
                ...
                href: 'NetWorkSpecialProject/DialogOP/ViewFaultOrderDetail_OP.aspx?id=' + id,
                buttons: btns
            });
        };
        //查询功能
        ...
        var exportFaultOrder = function () {
            jsPostForm('../ajax/Srv_NetWorkSpecialProject.ashx/ExportFaultOrder', $.serializeObject($('#searchForm')));
        };
        //生成领料单word
        var exportToWord = function () {
            ...
            if (!row) {
                ...
                jsPostForm('../ajax/Srv_NetWorkSpecialProject.ashx/ExportWordByID', { id: row.id });
        };
        //专项整治故障工单台账
        var orderGrid;
        $(function () {
            orderGrid = $('#orderGrid').datagrid({
                title: '专项整治故障工单台账',
                url: '../ajax/Srv_NetWorkSpecialProject.ashx/GetFaultOrder',
                striped: true,
                rownumbers: true,
                pagination: true,
                noheader: true,
...
<body class="easyui-layout">
        ...
        <form id="searchForm" style="margin: 0;">
            <table>
                <tr>
                    ...
                    <td>
                        <select id="cityname" class="combo easyui-combobox" style="width: 120px;" name="cityname" data-options="panelHeight:'auto',editable:false">
    ...
    <div data-options="region:'center',fit:true,border:false">
        <p class="sitepath">
            <b>当前位置：</b>专项整治管理 > <a href="javascript:void(0);">故障工单管理</a>
        </p>
