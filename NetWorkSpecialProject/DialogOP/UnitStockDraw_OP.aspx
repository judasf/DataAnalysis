...
<% 
    /** 
     * 专项整治物料物料领取
     *NSP_MaintainMaterial_StockDraw表操作对话框
     *
     */
...
<script type="text/javascript">
    ...
    var onFormSubmit = function ($dialog, $grid) {
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '您确定要提交领料信息？提交后则不能修改！', function (r) {
                if (r) {
                    var url = 'ajax/Srv_NetWorkSpecialProject.ashx/SaveUnitStockDrawInfo';
                    //判断领料单上传
                    if ($('#report').val() == "") {
    ...
    var addList = function () {
        var index = $('#index').val();
        index++;
        var insertEle = $('<tr align="center"><td> <input type="text" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="url:\'../ajax/Srv_NetWorkSpecialProject.ashx/GetClassInfoComboboxForStockOut\',valueField: \'id\',textField: \'text\',panelHeight:\'auto\',editable: false,required:true,onSelect:function(rec){ var url = \'../ajax/Srv_NetWorkSpecialProject.ashx/GetTypeInfoComboboxForStockOut?classname=\'+encodeURIComponent(rec.text);$(\'#typeid' + index + '\').combobox(\'setValue\',\'\').combobox(\'reload\', url); }"/> </td><td><input type="text" id="typeid' + index + '" name="typeid" style="width: 280px;" class="combo easyui-combobox" data-options="valueField: \'id\',textField: \'text\',editable: false,panelHeight: \'200\',required:true,onSelect:function(rec){setOrderStock(' + index + ',rec.id);}" /></td> <td><input type="text" id="storeorderno' + index + '" name="storeorderno" style="width: 160px;" /></td><td><input type="text" id="amount' + index + '" name="amount" class="inputBorder easyui-numberbox" style="width: 50px" data-options="min:0,required:true" onblur="checkStock(' + index + ',this);"/></td><td><label id="stock' + index + '"></label></td><td><label id="units' + index + '"></label></td><td><img src="../../Script/easyui/themes/icons/edit_remove.png" onclick="delList(this);" style="cursor: pointer;" /></td></tr>').insertBefore($('#memoTr'));
        $('#index').val(index);
        ...
        <tr>
            <td style="text-align: left" colspan="7">领料日期：
                ...
                <input id="lldw" type="text" name="lldw" style="width: 140px;" class="combo easyui-combobox" data-options="
                    ...
                    required:true,
                    panelHeight: 'auto',
                    url: 'ajax/Srv_NetWorkSpecialProject.ashx/GetNSP_MaintainMaterial_lldwCombobox',
                    onSelect:function(rec){ var url = '../ajax/Srv_NetWorkSpecialProject.ashx/GetllrComboboxForStockDraw?lldw='+encodeURIComponent(rec.text);$('#llr').combobox('setValue','').combobox('reload', url); }
                      " />
        ...
        <tr align="center">
            <td>
                ...
                <input type="text" class="combo easyui-combobox" name="classname" style="width: 100px;" data-options="
                    url:'../ajax/Srv_NetWorkSpecialProject.ashx/GetClassInfoComboboxForStockOut',
                    valueField: 'id',
                    textField: 'text',
                    panelHeight:'auto',
                    editable: false,
                    ...
                    onSelect:function(rec){
                     var url = '../ajax/Srv_NetWorkSpecialProject.ashx/GetTypeInfoComboboxForStockOut?classname='+encodeURIComponent(rec.text);
