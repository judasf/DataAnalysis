    ...
    <script type="text/javascript">
        ...
        var addRepairInfo = function () {
            var dialog = parent.$.modalDialog({
                ...
                height: 600,
                iconCls: 'icon-add',
                href: 'NetWorkSpecialProject/DialogOP/DailyRepairInfo_OP.aspx',
                buttons: [{
                    text: '提交',
                    handler: function () {
        ...
        var editRepairInfo = function (id) {
            var dialog = parent.$.modalDialog({
                ...
                height: 600,
                iconCls: 'icon-edit',
                href: 'NetWorkSpecialProject/DialogOP/DailyRepairInfo_OP.aspx?id=' + id,
                buttons: [{
                    text: '保存',
                    handler: function () {
                        parent.onFormSubmit(dialog, repairGrid);
        ...
        var viewDailyRepairDetail = function (id) {
            ...
            var dialog = parent.$.modalDialog({
                ...
                href: 'NetWorkSpecialProject/DialogOP/ViewDailyRepairDetail_OP.aspx?id=' + id,
                buttons: btns
            });
        };
        //导入数据
        var importResources = function () {
            var dialog = parent.$.modalDialog({
                ...
                href: 'NetWorkSpecialProject/DialogOP/ImporRepairInfo_op.aspx',
                buttons: [{
                    text: '导入',
                    handler: function () {
                        parent.onFormSubmit(dialog, repairGrid);
        ...
        var removeFun = function (orderno) {
            parent.$.messager.confirm('询问', '您确定要删除该维修台账？', function (r) {
                if (r) {
                    $.post('../ajax/Srv_NetWorkSpecialProject.ashx/RemoveRepairOrderByOrderNo', {
                        orderno: orderno
                    }, function (result) {
                        if (result.success) {
                            $.messager.show({
        ...
        var exportRepairInfo = function () {
            jsPostForm('../ajax/Srv_NetWorkSpecialProject.ashx/ExportDailyRepairInfo', $.serializeObject($('#searchForm')));
        };
        ...
        $(function () {
            repairGrid = $('#repairGrid').datagrid({
                title: '专项整治维修台账',
                url: '../ajax/Srv_NetWorkSpecialProject.ashx/GetDailyRepairInfo',
                striped: true,
                rownumbers: true,
                pagination: true,
                noheader: true,
...
<body class="easyui-layout">
    ...
    <div data-options="region:'center',fit:true,border:false">
        <p class="sitepath">
            <b>当前位置：</b>专项整治管理 > <a href="javascript:void(0);">维修台账</a>
        </p>
