    ...
    <script type="text/javascript">
        ...
        var addFun = function () {
            var dialog = parent.parent.$.modalDialog({
                ...
                height: 150,
                iconCls: 'icon-add',
                href: 'CustomAccess/dialogop/wlxh_OP.aspx', //将对话框内容添加到父页面
                buttons: [{
                    text: '添加',
                    handler: function () {
        ...
        var editFun = function (id) {
            var dialog = parent.$.modalDialog({
                ...
                height: 150,
                iconCls: 'icon-edit',
                href: 'CustomAccess/dialogop/wlxh_OP.aspx?id=' + id,
                buttons: [{
                    text: '保存',
                    handler: function () {
                        parent.onClassFormSubmit(dialog, grid);
        ...
        var removeFun = function (id) {
            parent.$.messager.confirm('询问', '您确定要删除此记录？', function (r) {
                if (r) {
                    $.post('../ajax/Srv_CustomAccess.ashx/RemovewlxhByID', {
                        id: id
                    }, function (result) {
                        if (result.success) {
        ...
        $(function () {
            grid = $('#grid').datagrid({
                title: '物料型号表',
                url: '../ajax/Srv_CustomAccess.ashx/GetCustomAccessWlxh',
                striped: true,
                rownumbers: true,
                pagination: true,
                singleSelect: true,
...
<body class="easyui-layout">
    ...
    <div id="toolbar" style="display: none; padding: 5px 10px 0;">
        <table style="table-layout: auto;">
            <tr>
                ...
                <%} %>
               <%-- <td>
                    <input id="searchBox" class="easyui-searchbox" style="width: 150px;" data-options="searcher:function(value,name){grid.datagrid('load',{where:'typename like \'%'+encodeURIComponent(value)+'%\''});},prompt:'请输入型号'" />
                </td>
                <td>
    ...
    <div data-options="region:'center',fit:true,border:false">
        <p class="sitepath">
            <b>当前位置：</b>客户接入事项管理 > <a href="javascript:void(0);">客户接入物料型号管理</a>
        </p>
