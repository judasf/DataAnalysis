...
<script type="text/javascript">
        ...
        if ($('form').form('validate')) {
            parent.$.messager.confirm('询问', '您确定要提交调拨信息？提交后则不能修改！', function (r) {
                if (r) {
                    //判断领料单上传
                    if ($('#report').val() == "") {
                        parent.$.messager.alert('提示', '请上传调拨单！', 'error');
                        return;
                    }
                    var url = 'ajax/Srv_ReuseMaintainMaterial.ashx/SaveReuseAllotStockInfo';
    ...
    $(function () {
        if ($('#id').val().length > 0) {
            ...
            $.post('../ajax/Srv_ReuseMaintainMaterial.ashx/GetReuseMaintainMaterialStockById', {
                ID: $('#id').val()
            }, function (result) {
                if (result.rows[0].id != undefined) {
                    ...
                    $('#allotunitid').combobox({
                        ...
                        panelHeight: 'auto',
                        required: true,
                        url: 'ajax/Srv_ReuseMaintainMaterial.ashx/GetAllotUnitInfoCombobox?id=' + encodeURIComponent(result.rows[0].unitname)
                    });
...
<form method="post" id="allotstockInfo">
    <table class="doc-table">
        ...
        <tr>
            <td style="text-align: right">利旧物料编号：</td>
