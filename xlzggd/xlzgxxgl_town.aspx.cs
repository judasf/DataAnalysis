...
{    
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                {
                    //提示未设置确认整改完结时间
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where isnull(wjsj,'')<>'' and isnull(qrwjsj,'')='' and len(id)>11  and pfdw='" + Session["deptname"].ToString() + "'");
                ...
                {
                    //提示未领料出库
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where zgll=1 and kgck=0 and len(id)>11  and pfdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = GetDataTable("select id,pfdw,pdr,pdsj,lxr,lxdh,whdw,fzr,zgqy,czwt,zgyq,zgsx,qywh,zgcs,zgr,tdyy,wjsj,qrwjsj,zgbz,fcyj,fcr,fcsj from xlzgxx  ");
