...
public partial class xlzgxxgl : System.Web.UI.Page
{    
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                if (Request.QueryString["action"] == "del")
                {
                    ...
                    {
                        DirectDataAccessor.Execute("Delete from xlzgxx where id='" + Request.QueryString["id"] + "'");
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('删除成功！'); location.href='xlzgxxgl.aspx';", true);
                ...
                {
                    //提示未设置确认整改完结时间
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where isnull(wjsj,'')<>'' and isnull(qrwjsj,'')='' and len(id)=11  and pdr='" + Session["uname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条整改未确认整改完结，请及时处理！')", true);
                ...
                {
                    //提示未领料出库
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where xgqr=1 and kgck=0 and len(id)=11  and pfdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条整改区域领料未出库，请及时处理！')", true);
                }
                ...
                {
                    //提示未指派区域维护
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where isnull(qywh,'')='' and len(id)=11 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条整改通知未派单，请及时处理！')", true);
                    //提示未确认区域维护领料
                    DataSet ds1 = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where  zgll=1 and wbqr=0 and len(id)=11 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds1.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds1.Tables[0].Rows[0][0].ToString() + "条整改未确认区域维护领料单，请及时处理！')", true);
                    //提示未设置完结时间
                    DataSet ds2 = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where kgck=1 and isnull(wjsj,'')='' and  len(id)=11 and whdw='" + Session["deptname"].ToString() + "'");
                    if (ds2.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds2.Tables[0].Rows[0][0].ToString() + "条整改未设置完结时间，请及时处理！')", true);
                }
                //区域维护
                if (Session["roleid"] != null && Session["roleid"].ToString() == "7")
                {
                    //提示未录入整改措施
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where isnull(zgcs,'')='' and len(id)=11 and qywh='" + Session["uname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条整改未录入整改措施，请及时处理！')", true);
                //区域维护提示未录入整改领料
                    DataSet ds1 = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where zgll=0 and len(id)=11 and isnull(zgcs,'')<>'' and qywh='" + Session["uname"].ToString() + "'");
                    if (ds1.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds1.Tables[0].Rows[0][0].ToString() + "条整改未领料，请及时处理！')", true);
                    //区域维护提示未设置整改完结
                    DataSet ds2 = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where kgck=1 and len(id)=11 and isnull(wjsj,'')='' and qywh='" + Session["uname"].ToString() + "'");
                    if (ds2.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds2.Tables[0].Rows[0][0].ToString() + "条整改未设置完结时间，请及时处理！')", true);
                ...
                {
                    //提示未确认区域维护领料单
                    DataSet ds = DirectDataAccessor.QueryForDataSet("select count(id)  from xlzgxx where  wbqr=1 and len(id)=11 and xgqr=0  and  zgtd=0 and pfdw='" + Session["deptname"].ToString() + "'");
                    if (ds.Tables[0].Rows[0][0].ToString() != "0")
                        ClientScript.RegisterStartupScript(this.GetType(), "info", "alert('您有" + ds.Tables[0].Rows[0][0].ToString() + "条整改未确认区域维护领料单，请及时处理！')", true);
    ...
    {
        string sql = "select pfdw from xlzgxx  where len(id)=11 ";
    ...
    {
        string sql = "select qywh from xlzgxx where len(id)=11 ";
        //检查角色：1：派单员，2：库管，8：线管(2014年7月15日 去掉2,8)
        if (Session["roleid"] != null && Session["deptname"] != null && Session["roleid"].ToString() == "1")
    ...
    private DataTable GetDataTable( string sqlStr)
    {
        ...
        //判断市县派单用户和库管，线管(2014年7月15日 去掉2,8)
        if (Session["roleid"] != null && Session["deptname"] != null && Session["roleid"].ToString() == "1")
        {
        ...
        if (Request.QueryString["gdzt"] != null)
        {
            ...
            switch (gdzt_str){
                case "wbpd":
                    whereStr += "  isnull(qywh,'')='' and";
                    break;
                case "zgcs":
                    whereStr += "  isnull(qywh,'')<>'' and isnull(zgcs,'')='' and";
                    break;
                case "qyll":
                    whereStr += " zgll=0 and isnull(zgcs,'')<>'' and";
                    break;
                case "wbqr":
                    whereStr += "  zgll=1 and wbqr=0 and";
                    break;
                case "xgqr":
                    whereStr += " wbqr=1 and xgqr=0  and  zgtd=0 and";
                    break;
                case "kgcl":
                        whereStr += " xgqr=1 and kgck=0 and";
                    break;
                case "wbzg":
    ...
    private void NewsBind()
    {
        ...
        if (Request.QueryString["qj"] != null)
        {
            condition += "&qj=" + Server.UrlEncode(Request.QueryString["qj"].ToString());
        }
        if (Request.QueryString["jz"] != null)
        {
            condition += "&jz=" + Server.UrlEncode(Request.QueryString["jz"].ToString());
        }
        if (Request.QueryString["dw"] != null)
        {
        ...
        if (Request.QueryString["gdzt"] != null)
        {
            condition += "&gdzt=" + Server.UrlEncode(Request.QueryString["gdzt"].ToString());
        }
        if (Request.QueryString["zgid"] != null)
        {
            condition += "&zgid=" + Server.UrlEncode(Request.QueryString["zgid"].ToString());
        }
        DataTable dt = GetDataTable("select * from xlzgxx  ");
        try
        {
    ...
    protected void btnExportExcel_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = GetDataTable("select id,pfdw,pdr,pdsj,lxr,lxdh,whdw,fzr,zgqy,czwt,zgyq,zgsx,qywh,zgcs,zgr,tdyy,wjsj,xllx,dbo.GetJobDuration(pdsj,wjsj),qrwjsj,zgbz,fcyj,fcr,fcsj from xlzgxx ");
        ...
        dt.Columns[16].ColumnName = "外包完结时间";
        dt.Columns[17].ColumnName = "线路类型";
        dt.Columns[18].ColumnName = "工单历时";
        dt.Columns[19].ColumnName = "确认完结时间";
        dt.Columns[20].ColumnName = "整改备注";
        dt.Columns[21].ColumnName = "复查意见";
        dt.Columns[22].ColumnName = "复查人";
        dt.Columns[23].ColumnName = "复查时间";
    ...
    /// <param name="qywh">区域维护</param>
    /// <returns></returns>
    public string handlePD(string id, string qywh)
    {
        string str="";
        if (qywh != "")
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看详情'> 已派单</a>";
        else
        {
            //角色判断，3：外包单位有权指定区域维护
            if (Session["roleid"] != null && Session["roleid"].ToString() == "3")
                str = "<a class='b_orange' href =xlzgqdqywh.aspx?zgid=" + id + "  title='点击为区域维护派单'>未派单</a>";
            else
                str = "<span class='b_orange'>未派单</span>";
        }
        return str;
    }
    /// <summary>
    ...
    /// <param name="zgcs">整改措施</param>
    /// <param name="qywh">区域维护</param>
    /// <returns></returns>
    public string handleZGCS(string id, string zgcs, string qywh)
    {
        string str = "";
        if (zgcs != "")//已录入整改措施
            str = "<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看整改详情信息'>已录入</a>";
        else //未录入整改措施
        {
            //角色判断，7：区域维护有权录入整改措施
            if (Session["roleid"] != null && Session["roleid"].ToString() == "7")
            {
                if (qywh == "")//未指定区域维护
                    str = "<a href=\"javascript:alert('该整改未指定区域维护，不能录入整改措施！');\" class='b_blue'>未录入</a> ";
                else //已指定区域维护
                    str = "<a class='b_blue' href=\"xlzgxxzgcs.aspx?id=" + id + "\" title='点击录入整改措施'>未录入</a>";
            }
            else //非区域维护
                str = "<span class='b_blue'>未录入</a>";
        }
    ...
    /// <returns></returns>
    public string handleQYLL(string id, string zgll, string zgcs)
   {
       string str="";
           if (zgll == "1")//已领料
           ...
           else //未领料
           {
               ...
               {
                   if (zgcs == "")//未录入整改措施
                       str = "<a href=\"javascript:alert('该整改未录入整改措施，不能领料！');\" class='b_blue'>未领料</a> ";
                   else //已录入整改措施
    ...
    /// <param name="wbqr">外包确认</param>
    /// <param name="zgll">区域维护领料</param>
    /// <returns></returns>
    public string handleWBQR(string id, string wbqr, string zgll)
   {
       string str = "";
       if (wbqr == "1")//外包已确认
       {
           str="<a class='gray'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看整改详情'>已确认</a>";
       }
       else
       {//外包未确认
           //角色判断，3：外包单位有权确认整改措施和区域维护领料信息
           if (Session["roleid"] != null && Session["roleid"].ToString() == "3")//是外包单位
           {
               if (zgll == "0")//未领料
                   str = "<a href=\"javascript:alert('该整改还未领料，不能进行领料工单确认！');\" class='b_green'>未确认</a> ";
               else
                   str = "<a class='b_green' href=\"xlzgxxwbllqr.aspx?zgid=" + id + "\" title='点击进行区域维护领料单确认'>未确认</a>";
           }
           else//非外包
               str = "<span class='b_green'>未确认</a>";
       }
       return str;
   }
    /// <summary>
    ...
    /// </summary>
    /// <param name="id">整改编号</param>
    /// <param name="xgqr">线管确认</param>
 /// <param name="zgtd">整改退单</param>
    /// <param name="wbqr">外包确认</param>
    /// <returns></returns>
    public string handleXGQR(string id, string xgqr,string zgtd, string wbqr)
    {
        string str = "";
        if (zgtd == "1")//区域维护领料单数据有误，线管退单
        {
            str = "<a class='b_red'   href=\"xlzgxxxq.aspx?zgid=" + id + "\"  title='点击查看整改详情'>已退单</a>";
        }
        else //线管未退单
        {
            if (xgqr == "1")//线管已确认
            ...
            {//线管未确认
                //角色判断，8：线路主管位有权对外包已确认的区域维护领料单进行确认
                if (Session["roleid"] != null && Session["roleid"].ToString() == "8")//是线路主管
                {
                    if (wbqr == "0")//外包单位未确认
                        str = "<a href=\"javascript:alert('外包单位没有对区域维护领料单进行确认，不能进行线路主管确认操作！');\" class='b_green'>未确认</a> ";
                    else
                        str = "<a class='b_green' href=\"xlzgxxxgllqr.aspx?zgid=" + id + "\" title='点击进行区域维护领料单确认'>未确认</a>";
                }
                else//非线路主管
                    str = "<span class='b_green'>未确认</a>";
            }
    ...
    /// <summary>
    /// 处理库管对区域维护领料单确认出库(市区需要线管确认)
    /// </summary>
    /// <param name="id">整改编号</param>
    /// <param name="kgck">库管确认出库</param>
    /// <param name="xgqr">线管确认</param>
    /// <returns></returns>
    public string handleKGQR(string id, string kgck, string xgqr)
    {
        string str = "";
        if (kgck == "1")//库管已确认出库
        {
        ...
        {//库管未确认出库
            ...
            {
                if (xgqr == "0")//线管未确认
                    str = "<a href=\"javascript:alert('线路主管没有对区域维护领料单进行确认，不能进行出库操作！');\" class='b_lightblue'>未出库</a> ";
                else
    ...
    /// <param name="id">整改编号</param>
    /// <param name="kgck">库管确认出库</param>
    /// <param name="wbqr">线管确认</param>
    /// <returns></returns>
    public string handleKGQR_town(string id, string kgck, string wbqr)
    {
        string str = "";
        if (kgck == "1")//库管已确认出库
        {
        ...
        {//库管未确认出库
            ...
            {
                if (wbqr == "0")//外包未确认
                    str = "<a href=\"javascript:alert('外包单位没有对区域维护领料单进行确认，不能进行出库操作！');\" class='b_lightblue'>未出库</a> ";
                else
                    str = "<a class='b_lightblue' href=\"xlzgxxkgllck.aspx?zgid=" + id + "\" title='点击进行区域维护领料单进行出库操作'>未出库</a>";
            }
            else//非库管
    ...
    /// <param name="xgqr">线管确认</param>
    /// <returns></returns>
    public string handleWBZGWJ(string id, string wjsj, string xgqr)
    {
        string str = "";
        if (wjsj != "")//外包已录入完结时间
        {
        ...
        {//外包未确认
            ...
            {
                if (xgqr == "0")//领料未出库
                    str = "<a href=\"javascript:alert('线路主管没有对区域维护领料单进行确认，不能进行设置整改完结！');\" class='b_green'>未整改</a> ";
                else
