...
public partial class xlbdxxtj : System.Web.UI.Page
{
    ...
    {
        DataSet ds = DirectDataAccessor.QueryForDataSet("select distinct DateName(year,pdsj)as yearstr from xlzgxx ");
    ...
    {
        string sql = "select pfdw from xlzgxx ";
    ...
    private string GetSqlStr()
    {
        ...
        for (int i = 1; i < 13; i++)
        {
            ...
            joinStr.Append("select pfdw,count(id) as num  from xlzgxx ");
            joinStr.Append(whereStr);
            joinStr.Append(" group by pfdw,substring(pdsj,0,8) )");
            joinStr.Append(" as m" + s + " on a.pfdw=m" + s + ".pfdw ");
        }
        sql.Append(zdStr);
        sql.Append(" from (select pfdw from xlzgxx group by pfdw) as a ");
        sql.Append(joinStr);
        sql.Append(dwStr);
        return sql.ToString();
    }
