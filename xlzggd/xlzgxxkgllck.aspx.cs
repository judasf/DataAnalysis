...
public partial class xlbdxxxq : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = DirectDataAccessor.QueryForDataSet("select amount,classname,typename,panhao from xlzgxx_llmx where zgid='" + zgid.InnerText + "'");
        ...
        sql.Append("update xlzgxx set kgck=1,kgcksj='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'  where id='" + zgid.InnerText + "';");
        //处理退料
        if(ddltlzt.Text=="1")
        {
            sql.Append("insert into xlzgxx_tlmx values('" + zgid.InnerText + "','无');");
            sql.Append("update xlzgxx set zgtl=1 where id='" + zgid.InnerText + "';");
        }
        //写入sql日志
        DirectDataAccessor.writeLog("ZG_KG", Session["pre"].ToString() == "" ? "SQ_" : Session["pre"].ToString(), sql.ToString());
